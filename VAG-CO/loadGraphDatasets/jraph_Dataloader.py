...
class JraphSolutionDataset_fromMemory(Dataset):
    ...
    def get_dataset(self, cfg, mode = "", seed = None):
        ...
        path = os.path.join(cfg["Paths"]["work_path"],"loadGraphDatasets", "DatasetSolutions", "no_norm_H_graph", self.dataset_name, f"{mode}_{EnergyFunction}_seed_{seed}_solutions.pickle")
...
class JraphSolutionDataset(InMemoryDataset):
    ...
    def get_dataset_paths(self, cfg, mode="", seed=None):
        ...
        self.base_path = os.path.join(cfg["Paths"]["work_path"], "loadGraphDatasets", "DatasetSolutions", "no_norm_H_graph",
    ...
    def __getitem__(self, idx):
        ...
        file = open(self.base_path + f"/_idx_{self.indices[idx]}.pickle", "rb")
        ...
        igraph["gt_Energy"] = np.array([graph_dict["normed_Energies"]])
