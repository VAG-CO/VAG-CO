...
def generate_and_solve_regular_graphs(n_graphs = 100, num_nodes = 100, k = 10, EnergyFunction = "MIS", mode = "val", seed = 0, parent = True):
    ...
    for idx in range(n_graphs):
        ...
        if(k == "all"):
            curr_k = np.random.choice([3,7,10,20])
            n_range = int(0.1*num_nodes)
            curr_num_nodes = np.random.choice(num_nodes - 2*np.arange(-n_range,n_range))
        else:
...
def make_dataset(parent = True):
    modes = ["train", "val","test"]
    n_graph_list = [3000, 100, 1000]
    num_node_list = [100]
    for num_nodes in num_node_list:
            for mode, n_graphs in zip(modes, n_graph_list):
                generate_and_solve_regular_graphs(n_graphs = n_graphs, num_nodes=num_nodes, k = "all", mode = mode, parent = parent, seed = 123)
...
if(__name__ == "__main__"):
    load_and_greedy_solve(num_nodes = 1000, ks = ["all"], mode = "val")
    #make_dataset()
