...
class DataContainer:
    ...
    def __init__(self, cfg,  time_horizon, Nb, num_spins, lam, n_sampled_spins = 1):
        ...
        self.values = np.zeros((self.max_steps+1, Nb))
        self.rewards = np.zeros((self.max_steps, Nb))
        self.log_probs = np.zeros((self.max_steps , Nb))
        self.actions = np.zeros((self.max_steps, Nb))
        self.external_fields = np.zeros((self.max_steps, Nb))
    ...
    def append(self, Jgraph, external_fields, values, rewards, log_probs, actions):
