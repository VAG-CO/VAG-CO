...
from trainPPO.IGraphVecEnv.GraphEnv_train_configuration import IGraphEnv as RB_IGraphEnv
...
class IGraphEnv(RB_IGraphEnv):
    ...
    def step(self, data):
        ...
        if(self.dones):
            ...
            if(self.mode == "test" or self.EnergyFunction == "MaxCl_EGN"):
                ...
                if(self.EnergyFunction == "MaxCl_EGN"):
                    pass
                else:
                    self.finished_energy = self.compute_Energy_full_graph(self.Nb_spins)
...
def envstep():
    ...
    cfg["Ising_params"]["EnergyFunction"] = "MaxCl_EGN"
...
def run(SpinEnv, Hb, Nb, n_sampled_sites):
    ...
    while(True):
        ...
        start = time.time()
        done, reward, terminated, H_graph_dict = SpinEnv.step(action)
        #print("reward", reward)
