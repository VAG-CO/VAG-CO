    ...
    def create_dict(self, datacontainer, N_idxs, S_idxs):
        ...
        for key in data_dict:
            if (key != "graphs" and key != "external_fields" and key != "compl_graphs"):
                ...
                res = np.reshape(minib_ext_fields, (len(N_idxs), int(minib_ext_fields.shape[0] / len(N_idxs)), 1))
                minib_dict[key] = res
...
def collate_data_dict(Hb_data_dict):
    ...
    return minib_H_graphs, None, minib_actions, minib_A_k, minib_log_probs, minib_value_target, Hb_ext_field_list
