...
class HiVNAPPo:
    def __init__(self):
        ...
        self.vmapped_forward_env = jax.vmap(self.forward_env, in_axes = (None, None, None, 0,0), out_axes = (0,0,0,0, 0))
    ...
    def collate_data_dict(self, Hb_data_dict):
        ...
        return minib_H_graphs, minib_actions, minib_A_k, minib_log_probs, minib_value_target, Hb_ext_field_arr
    ...
    def init_attributes(self, config):
        ...
        self.edge_p = config["Ising_params"]["edge_p"]
        ...
        # try:
        #     print("config is old")
        #     print("encoder will be overwritten")
        #     encode_MLP_features = config["Network_params"]["encode_MLP_features"]
        #     config["Network_params"]["encode_node_features"] = encode_MLP_features
        #     config["Network_params"]["encode_edge_features"] = encode_MLP_features
        #     config["Network_params"]["GNN_name"] = "old_encoder"
        # except:
    ...
    def train(self, config, load_path = None, reload_epoch = None):
        ...
        run = wandb.init(project=project_name, reinit=True, group=group, job_type=job_type, config=config)#, settings=wandb.Settings(start_method='fork'))
        ...
        if(load_path != None and reload_epoch == None):
            print("Load model from checkpoint")
            params, opt_state = LoadUtils.load_model(load_path)
            ...
            print("Load model from checkpoint at epoch", reload_epoch)
    ...
    def train_loop(self, RNN, params, opt_state, Nb_RNN_key, epoch_arr, epochs, batch_epochs, N_warmup, N_equil, N_anneal, anneal_scheduler, T, run_path):
        ...
        for epoch in tqdm(epoch_arr):
            ...
            start_env = time.time()
            DataContainerList, t_Hb_Nb_Energy_per_action, t_Hb_Nb_log_prob, t_Hb_Nb_rewards, Nb_RNN_key = self.make_env_steps(params, RNN.T, Nb_RNN_key)
            end_env = time.time()
            ...
            log_dict = {}
            ### TODO log metrics as mean over batch epochs
            ...
            log_dict["train/mean_action_prob"] =  cpu_np.mean(cpu_np.mean(cpu_np.exp(t_Hb_Nb_log_prob), axis = 0))
            ...
            if(epoch % val_epoch == 0 or epoch == epochs -1 or epoch == 0 or (100*RNN.T/T < 2 and epoch % finer_val_epoch == 0) ):
                ...
                if(self.cfg["Save_settings"]["save_params"] == True):
                    if(self.cfg["Save_settings"]["save_mode"] != "best"):
                        ...
                        if (best_rel_error < best_metrics["best_rel_error"]):
                            best_metrics["best_rel_error"] = best_rel_error
                            SaveModel.save_best_model_params(params, opt_state, "val_best_rel_error", run_path)
                        if (rel_error < best_metrics["rel_error"]):
                            best_metrics["rel_error"] = rel_error
    ...
    def train_on_minibatches_Dataset(self, RNN, params, opt_state, loss_dict):
        ...
        for Cidx , (minib_H_graphs, minib_actions, minib_A_k, minib_log_probs, minib_value_target, concatenated_Nb_external_fields) in enumerate(self.ContainerDataloader):
            end_time = time.time()
            ...
            padded_minib_H_graph = jutils.cast_Tuple_to(padded_minib_H_graph, np_=jnp)

            start_jit_time = time.time()

            padded_minib_compl_graphs = padded_minib_H_graph
            padded_minib_H_graph = jutils.cast_Tuple_to(padded_minib_H_graph, np_=jnp)
                # print("padded n_node")
                # print(padded_minib_H_graph.n_node)

            params, opt_state, (overall_loss, (actor_loss, critic_loss)) = RNN.backward_sparse_reduced_Nb_Hb_loss(params, opt_state, padded_minib_H_graph, padded_minib_compl_graphs, padded_concatenated_Nb_external_fields)


            end_jit_time = time.time()
            print("jit_time", end_jit_time - start_jit_time)
            del padded_minib_H_graph
            del padded_minib_compl_graphs
    ...
    def init_TestEnv(self):
        self.TestSpinEnv = SubprocVecEnv([myfunc(self.cfg, H_seed, mode = "test") for H_seed in range(self.n_test_graphs)])
        pass
    ...
    def eval_on_testdata(self, params, cfg, n_test_graphs, padding_factor = 2., n_perm = 0, Nb = 8, mode = "perm"): # modes = perm, perm+Nb, perm+beam_search, Nb, beam_search
        self.n_test_graphs = n_test_graphs
        if(mode == "normal"):
            self.Nb = Nb
    ...
    def make_env_steps(self, params, T, RNN_key):
        ...
        for i in range(self.time_horizon + 1):
            ...
            padded_batched_H_graph = jutils.cast_Tuple_to(padded_batched_H_graph, np_=gpu_np)


            padded_concatenated_Nb_external_fields = jnp.array(padded_concatenated_Nb_external_fields)

            value, log_probs, sampled_bin_values, log_probs_classes, RNN_key = self.jitted_vmapped_forward_env(params, padded_batched_H_graph,padded_Hb_compl_graphs, padded_concatenated_Nb_external_fields, RNN_key)

            sampled_bin_values = cpu_np.reshape(sampled_bin_values, (self.Nb, self.Hb, 1))
            action = sampled_bin_values
            Hb_Nb_actions = np.swapaxes(action, 0,1)
            Nb_Hb_values = cpu_np.reshape(value, (self.Nb, self.Hb,1))
            Hb_Nb_values = np.swapaxes(Nb_Hb_values,0,1)

            Nb_Hb_log_probs = cpu_np.reshape(log_probs, (self.Nb, self.Hb,1))
            Hb_Nb_log_probs = np.swapaxes(Nb_Hb_log_probs,0,1)
            Hb_Nb_Temperatures = T*np.ones_like(Hb_Nb_actions)

            data = np.concatenate([Hb_Nb_actions, Hb_Nb_values, Hb_Nb_log_probs, Hb_Nb_Temperatures], axis=-1)
        ...
        return DataContainerList, t_Hb_Nb_Energy_per_action, t_Hb_Nb_log_prob, t_Hb_Nb_rewards, RNN_key
    ...
    def make_evaluation_steps(self, params, RNN_key, SpinEnv, log_dict, mode = "val", sampling_mode = "normal"):
        ...
        while(not finished):
            ...
            value, log_probs, sampled_bin_values, log_probs_classes, RNN_key = self.jitted_vmapped_forward_env(params, padded_batched_H_graph, padded_val_Hb_compl_graphs,padded_concatenated_Nb_external_fields ,
                                                                                                  RNN_key)
            if(sampling_mode == "greedy"):
                ### TODO implement beam search
                # print("n_graphs finished", len(Energy_list))
                # print(sampled_bin_values)
                sampled_bin_values = np.argmax(log_probs_classes, axis = -1)
                ...
                # print("n_graphs finished", len(Energy_list))
                # print(sampled_bin_values)
            elif(sampling_mode == "beam_search"):
                ### TODO pay attention beam search is not implemented yet
                log_probs_classes = cpu_np.reshape(log_probs_classes, (self.Nb, self.n_test_graphs, self.n_classes))
                log_probs_classes = np.swapaxes(log_probs_classes, 0,1)
                Hb_Nb_actions = log_probs_classes
            else:
                sampled_bin_values = cpu_np.reshape(sampled_bin_values, (self.Nb, self.n_test_graphs, 1))
                Hb_Nb_actions = np.swapaxes(sampled_bin_values, 0,1)
    ...
    def make_env_steps_test(self, params, RNN_key, log_dict, sampling_mode = "beam_search"):
        ...
        if(sampling_mode != "CE"):
            self.make_evaluation_steps(params, RNN_key, self.TestSpinEnv, log_dict, mode = "test", sampling_mode = sampling_mode)
        else:
