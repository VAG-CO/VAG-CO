...
def solveMVC_as_MIP(H_graph, time_limit = float("inf")):
...
def solveMIS_as_MIP(H_graph,  time_limit = float("inf")):
...
def solveMaxCut(H_graph, time_limit=float("inf"), solution_limit=None, verbose=0, measure_time = False, bnb = True, model_name = "mip1"):
    N = int(H_graph.n_node[0])
    senders = H_graph.senders
    receivers = H_graph.receivers
    edges = H_graph.edges
    ...
    obj1 = g.quicksum( (2*var_dict[int(s)]-1) *weight* (2*var_dict[int(r)]-1)/2 for s, r, weight in zip(senders, receivers, edges))
