...
def from_jgraph_to_igraph(jgraph):
    senders = jgraph.senders
    receivers = jgraph.receivers
    Energy = jgraph.globals
    ig = igraph.Graph([ (s,r) for s,r in zip(senders, receivers)])
    ig.simplify()
    ig["gt_Energy"] = Energy
    return ig

def from_jgraph_to_igraph_normed(jgraph):
    senders = jgraph.senders
    receivers = jgraph.receivers
    Energy = jgraph.globals
...
def from_igraph_to_jgraph(igraph):
...
def pad_graph_and_ext_fields_to_nearest_power_of_k(graphs_tuple: jraph.GraphsTuple, Nb_ext_fields, k = 1.4, np_ = jnp, min_nodes = 1) -> jraph.GraphsTuple:
    ...
    padded_ext_fields = np.concatenate([Nb_ext_fields, np.zeros((Nb_ext_fields.shape[0], pad_nodes_to-Nb_ext_fields.shape[1], 1))], axis = -2)
    return jraph.pad_with_graphs(graphs_tuple, pad_nodes_to, pad_edges_to,
