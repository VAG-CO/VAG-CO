from Networks.BuildingBlocks.EncodeProcessDecodeGNNs import EncodeProcess, EncodeProcessNew, EncodeProcess_node_embedding
from Networks.BuildingBlocks.GNNetworks import ProbMLP, ValueMLP
...
class GNN_actor_critic(nn.Module):
    ...
    def setup(self):
        ...
        if(self.GNN_name == "newEncoder"):
            self.prob_GNN = EncodeProcessNew(self.message_MLP_features, self.node_MLP_features, self.edge_MLP_features, self.encode_node_features, self.encode_edge_features, layer_norm = self.layer_norm,
                                               ...
                                               message_passing = self.GNN_mode, training = self.training, weight_tied = False)
        elif(self.GNN_name == "newEncoder_embedding"):
    ...
    def forward_padded(self, H_graph, compl_H_graph, spin_sites):
        ...
        if(self.policy_global_features):
            ...
            log_prob = self.probMLP(concat_node_embeddings)
    ...
    def sample_unpadded(self, H_graph, compl_H_graph, Ext_fields, key):
        ...
        values, log_probs = self.forward(H_graph, compl_H_graph, spin_sites)
        ...
        return values, spin_log_prob, sampled_classes, log_probs, key

    def sample(self, H_graph, compl_H_graph, Ext_fields, key):
        H_graph, spin_sites = self._make_H_graph_input(H_graph, Ext_fields)

        values, log_probs = self.forward_padded(H_graph, compl_H_graph, spin_sites)
        ...
        return values[:-1], spin_log_prob[:-1], sampled_classes[:-1], log_probs[:-1], key
