...
class ResidualNonlinearMessageLayer(nn.Module):
    ...
    def __call__(self, graph: jraph.GraphsTuple) -> jraph.GraphsTuple:
        ...
        ### TODO add or remove mean aggregator, it cause NANs
        # num_neighbours = jax.ops.segment_sum(jnp.ones_like(messages), receivers, total_num_nodes)
        # num_neighbours = jnp.clip(num_neighbours, a_min= 1. )
        # aggr_messages = aggr_messages/num_neighbours
