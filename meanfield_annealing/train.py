...
class TrainMeanField:
	def __init__(self, config, wandb_project, wandb_group, wandb_run):
		...
		self.epochs = self.config["epochs"] if not self.config["annealing"] else self.config["N_warmup"] + self.config[
			"N_anneal"] + self.config["N_equil"]
		...
		# Loss function
		self.loss = self.loss_energy if not self.config["annealing"] else self.loss_free_energy
	...
	def loss_energy(self, params, graphs, T, key):
		...
		"""
		states, _log_probs, spin_log_probs, spin_logits, key = self.model.apply(params, graphs, self.N_basis_states, key)
	...
	def loss_free_energy(self, params, graphs, T, key):
		...
		# print(f"delta_free_energy.shape: {delta_free_energy.shape}")
		# print(log_probs.T)
		# print(f"log_probs.T.shape: {log_probs.T.shape}")
		# temp = lax.stop_gradient(delta_free_energy) * log_probs.T
	...
	def eval(self, epoch):
		...
		mean_APR = []
		mean_best_APR = []
		for iter, (graph_batch, gt_normed_energies, gt_spin_states) in enumerate(self.dataloader_val):
			...
			states, log_probs, spin_log_probs, spin_logits, self.key = self.model.apply(self.params, graph_batch,
																		   self.N_basis_states,
																		   self.key)
			...
			mean_energy, mean_normed_energy, mean_normed_free_energy, mean_gt_energy, mean_best_energy, rel_error, mean_best_rel_error, mean_prob, APR, best_APR = self.__calculate_reporting(
				energies[:-1], gt_normed_energies, spin_log_probs, free_energies)
		...
		elif epoch % self.save_modulo == 0 or self.T == 0.:
